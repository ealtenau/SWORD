...
def migrate_region(
    ...
    """
    from src._legacy.updates.sword_utils import read_nc
    ...
    cl_neighbor_count = _migrate_centerline_neighbors(
        conn, centerlines, region, batch_size
    )
...
def _migrate_centerlines(
    conn, centerlines, region: str, version: str, batch_size: int
) -> int:
    """
...
def _migrate_centerline_neighbors(
    conn, centerlines, region: str, batch_size: int
) -> int:
    """
...
def build_all_geometry(
    db: SWORDDatabase, regions: list = None, verbose: bool = True
) -> None:
    """
...
def validate_migration(
    nc_path: str, db: SWORDDatabase, region: str, sample_size: int = 1000
) -> dict:
    """
    ...
    for table, nc_count in nc_counts.items():
        db_count = conn.execute(
            f"SELECT COUNT(*) FROM {table} WHERE region = ?", [region]
        ).fetchone()[0]
    ...
    sample_indices = np.random.choice(
        len(reaches.id), min(sample_size, len(reaches.id)), replace=False
    )

    for idx in sample_indices[:10]:  # Detailed check on 10 samples
        reach_id = reaches.id[idx]
        db_row = conn.execute(
            "SELECT * FROM reaches WHERE reach_id = ?", [int(reach_id)]
        ).fetchdf()

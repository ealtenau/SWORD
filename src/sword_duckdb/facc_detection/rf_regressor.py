...
class FaccRegressor:
    ...
    EXCLUDE_FEATURES_DEFAULT = [
        ...
        "avg_fwr",
        "ratio_to_median",
        "fwr_upstream_consistency",
        "fwr_network_zscore",
        "network_avg_fwr",
        "network_std_fwr",
        "side_channel_high_fwr",
    ...
    EXCLUDE_FEATURES_NO_FACC = EXCLUDE_FEATURES_DEFAULT + [
        ...
        "max_3hop_upstream_facc",
        "max_3hop_downstream_facc",
        "max_5hop_upstream_facc",
        "max_5hop_downstream_facc",
        "facc_3hop_ratio",
        "facc_5hop_ratio",
        # All FWR features (facc/width derived)
        "fwr_network_ratio",
        "fwr_per_path_freq",
        "fwr_downstream_consistency",
        "fwr_propagation",
        "avg_upstream_fwr",
        "avg_downstream_fwr",
        "avg_2hop_upstream_fwr",
        "avg_2hop_downstream_fwr",
        "avg_3hop_upstream_fwr",
        "avg_3hop_downstream_fwr",
        "avg_5hop_upstream_fwr",
        "avg_5hop_downstream_fwr",
    ...
    def fit(self, df: pd.DataFrame, test_size: float = 0.2) -> dict:
        """Train the regressor."""
        mode = (
            "NO-FACC (topology/width only)"
            if self.exclude_facc_features
            else "standard"
        )
        ...
        mask = y_test_orig > 0
        pct_errors = (
            np.abs(y_pred[mask] - y_test_orig.values[mask])
            / y_test_orig.values[mask]
            * 100
        )
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result["facc_ratio"] = result["predicted_facc"] / result["facc"].replace(
            0, np.nan
        )
...
class SplitFaccRegressor:
    ...
    def __init__(
        ...
        self.exclude_facc_features = exclude_facc_features
        self._exclude_list = (
            FaccRegressor.EXCLUDE_FEATURES_NO_FACC
            if exclude_facc_features
    ...
    def fit(self, df: pd.DataFrame, test_size: float = 0.2) -> Dict[str, Any]:
        ...
        X_train, X_test, y_train, y_test, ms_train, ms_test, lf_train, lf_test = (
            train_test_split(
                X,
                y_log,
                main_side,
        ...
        for ms in [0, 1, 2]:
            for lf in [0, 1, 2]:
                ...
                if n_samples < self.min_samples_per_group:
                    print(
                        f"  Group ({ms}, {lf}): {n_samples} samples - skipping (< {self.min_samples_per_group})"
                    )
                    continue
                ...
                if mask_test.sum() > 0:
                    ...
                    if valid.sum() > 0:
                        pct_err = (
                            np.abs(y_pred[valid] - y_true.values[valid])
                            / y_true.values[valid]
                            * 100
                        )
                        self.group_metrics[(ms, lf)]["median_pct_error"] = np.median(
                            pct_err
                        )
                        self.group_metrics[(ms, lf)]["p90_pct_error"] = np.percentile(
                            pct_err, 90
                        )

                    print(
                        f"  Group ({ms}, {lf}): n={n_samples:,}, R²={self.group_metrics[(ms, lf)]['r2']:.4f}, "
        ...
        valid = y_test_orig > 0
        pct_errors = (
            np.abs(y_pred_all[valid] - y_test_orig.values[valid])
            / y_test_orig.values[valid]
            * 100
        )
        ...
        return {"overall": self.overall_metrics, "by_group": self.group_metrics}
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        main_side = (
            df["main_side"].values if "main_side" in df.columns else np.zeros(len(df))
        )
    ...
    def get_feature_importance(
        self, group: Optional[Tuple[int, int]] = None
    ) -> pd.DataFrame:
        ...
        if group is not None:
            ...
            importance = pd.DataFrame(
                {"feature": self.feature_names, "importance": all_importances}
            )

...
def extract_features_with_enhancements(db_path: str, output_path: Path) -> pd.DataFrame:
    ...
    print(
        f"Saved {len(features):,} rows with {len(features.columns)} columns to {output_path}"
    )
...
def train_and_compare(
    ...
    if not skip_baseline:
        ...
        baseline = FaccRegressor(
            n_estimators=100, max_depth=20, exclude_facc_features=exclude_facc_features
        )
        ...
        baseline_importance.to_csv(
            output_dir / f"rf_regressor_baseline{suffix}_importance.csv", index=False
        )
    ...
    split_importance.to_csv(
        output_dir / f"rf_split_regressor{suffix}_importance.csv", index=False
    )
    ...
    if not skip_baseline:
        ...
        baseline_preds.to_csv(
            output_dir / f"rf_baseline_predictions{suffix}.csv", index=False
        )
    ...
    print(
        f"  Median split predicted: {split_preds['predicted_facc'].median():,.0f} km²"
    )
    print(f"  Median ratio: {split_preds['facc_ratio'].median():.2f}x")

    if not skip_baseline:
        print(
            f"  Median baseline predicted: {baseline_preds['predicted_facc'].median():,.0f} km²"
        )
    ...
    for model, count in model_counts.items():
        ...
        print(
            f"  {model}: n={count}, median_ratio={subset['facc_ratio'].median():.2f}x"
        )
        ...
        comparison.update(
            {
                ...
                "r2_improvement": results["split"]["overall"]["r2"]
                - results["baseline"]["r2"],
                "error_reduction": results["baseline"]["median_pct_error"]
                - results["split"]["overall"]["median_pct_error"],
            }
        )
...
def main():
    ...
    parser.add_argument(
        "--features", help="Path to existing features CSV (skip extraction)"
    )
    parser.add_argument("--anomalies", help="Path to anomalies GeoJSON")
    parser.add_argument(
        "--skip-baseline", action="store_true", help="Skip baseline training"
    )
    parser.add_argument(
    ...
    # Anomalies path
    anomalies_path = (
        Path(args.anomalies) if args.anomalies else output_dir / "all_anomalies.geojson"
    )
    ...
    if "baseline" in results:
        ...
        print(
            f"Baseline median % error: {results['baseline']['median_pct_error']:.1f}%"
        )

    print(f"Split model R²: {results['split']['overall']['r2']:.4f}")
    print(
        f"Split model median % error: {results['split']['overall']['median_pct_error']:.1f}%"
    )

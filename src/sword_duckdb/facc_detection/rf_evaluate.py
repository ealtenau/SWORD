...
class RFEvaluator:
    ...
    def compare_to_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overlap_pct_of_rf": 100 * len(overlap) / len(rf_positives)
            if rf_positives
            else 0,
            "overlap_pct_of_rules": 100 * len(overlap) / len(rule_positives)
            if rule_positives
            else 0,
            "agreement_rate": 100
            * (len(overlap) + len(df) - len(rf_positives.union(rule_positives)))
            / len(df),
    ...
    def analyze_seed_coverage(self) -> Dict[str, Any]:
        ...
        rf_detected_seeds = seeds_in_data[seeds_in_data["rf_prediction"] == 1][
            "reach_id"
        ].tolist()
        rf_missed_seeds = seeds_in_data[seeds_in_data["rf_prediction"] == 0][
            "reach_id"
        ].tolist()

        # Compare to rule detection
        rule_detected_seeds = seeds_in_data[seeds_in_data["is_rule_anomaly"] == 1][
            "reach_id"
        ].tolist()
        ...
        return {
            ...
            "rf_recall": len(rf_detected_seeds) / len(seeds_in_data)
            if len(seeds_in_data) > 0
            else 0,
            "rule_detected": len(rule_detected_seeds),
            "rule_recall": len(rule_detected_seeds) / len(seeds_in_data)
            if len(seeds_in_data) > 0
            else 0,
            "rf_detected_ids": rf_detected_seeds,
            "rf_missed_ids": rf_missed_seeds,
            "rf_catches_missed_rules": [
                s for s in MISSED_SEEDS if s in rf_detected_seeds
            ],
        }

    def get_novel_anomalies(
        self, min_probability: float = 0.7, max_results: int = 100
    ) -> pd.DataFrame:
        """
    ...
    def get_false_negatives(self, max_results: int = 50) -> pd.DataFrame:
        ...
        fn = df[(df["rf_prediction"] == 0) & (df["is_rule_anomaly"] == 1)].copy()
    ...
    def _write_markdown_report(self, report: Dict[str, Any], path: Path):
        ...
        for seed_info in report["missed_seeds_analysis"]:
            ...
            lines.append(
                f"- **{reach_id}** ({region}): {status} (prob={prob:.3f}) {note}"
            )
    ...
    def _export_geojson(self, df: pd.DataFrame, path: Path, description: str):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate RF classifier for facc anomaly detection"
    )
    parser.add_argument(
        "--predictions", required=True, help="Path to rf_predictions.parquet"
    )

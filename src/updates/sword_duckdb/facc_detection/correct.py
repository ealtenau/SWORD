...
class FaccCorrector:
    ...
    def estimate_corrections(
        ...
        """
        Compute facc_corrected with edge case handling.

        Processing order (topologically sorted):
        1. orphan_entry: regression only (no upstream)
        2. entry_point: regression estimate (upstream is clean)
            ...
            for fix_type in ['orphan_entry', 'entry_point', 'propagation']:
                ...
                for _, row in type_anomalies.iterrows():
                    ...
                    if fix_type == 'orphan_entry':
                        # Headwater: use regression only
                        facc_corrected = local_estimate
                    elif fix_type == 'entry_point':
                        # Entry point: upstream is clean, use regression for this reach
                        facc_corrected = local_estimate
                    else:
                        # Propagation: upstream might be corrected already
...
def correct_facc_anomalies(
    ...
    with FaccCorrector(db_path, read_only=dry_run) as corrector:
        ...
        corrections = corrector.estimate_corrections(classified, region=region)

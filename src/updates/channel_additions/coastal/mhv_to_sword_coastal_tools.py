...
(mhv_to_sword_coastal_tools.py)
==============================================

Utilities for adding MHV coastal rivers to 
SWORD. 
...
def read_mhv_sword(mhv_fn):
    ...
    if 'add_coast' in mhv.groups['centerlines'].variables.keys():
        ...
        data.add_flag = np.array(mhv['/centerlines/add_coast'][:])
        data.network = np.array(mhv['/centerlines/network_all'][:])
        
        # zero out entire segments that have any zero basin values. 
        rmv_segs = np.unique(data.seg[np.where(data.basins == 0)[0]])
        rmv = np.where(np.in1d(data.seg, rmv_segs)==True)[0]
        data.basins[rmv] = 0
        # keep all other flagged segments.
        keep = np.where((data.add_flag > 0)&(data.basins > 0))[0]

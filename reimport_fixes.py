...
============================
Reimport fixes exported from the QA reviewer into another database.
...
def main():
    ...
    if args.dry_run:
        ...
        for _, fix in actual_fixes.iterrows():
            print(
                f"  {fix['check_id']}: reach {fix['reach_id']} ({fix['region']}): {fix['column_changed']} = {fix['new_value']}"
            )
        return
    ...
    for region in actual_fixes["region"].unique():
        ...
        with workflow.transaction(
            f"Reimport {len(region_fixes)} fixes from {args.csv}"
        ):
            for _, fix in region_fixes.iterrows():
                ...
                if kwargs:
                    ...
                    workflow.modify_reach(fix["reach_id"], reason=reason, **kwargs)

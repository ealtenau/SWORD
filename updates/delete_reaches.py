...
def read_data(filename):
    ...
    #reaches.lake_id = data.groups['reaches'].variables['lake_id'][:]
...
region = 'AS'
version = 'v14'
sword_dir = '/Users/ealteanau/Documents/SWORD_Dev/outputs/'+version+'/netcdf/'+region.lower()+'_sword_'+version+'.nc'
rch_dir = '/Users/ealteanau/Documents/SWORD_Dev/update_requests/v13/completed/AS_Reach_Removal.csv'

rm_rch = pd.read_csv(rch_dir)
centerlines, nodes, reaches = read_data(sword_dir)
rch_check = reaches.id

#### LOOP
for ind in list(range(len(rm_rch.reach_id))):
    rch_ind = np.where(reaches.id == rm_rch.reach_id[ind])[0]
    node_ind = np.where(nodes.reach_id == rm_rch.reach_id[ind])[0]
    cl_ind = np.where(centerlines.reach_id[0,:] == rm_rch.reach_id[ind])[0]
...
# version = 'v14'
# sword_dir = '/Users/ealteanau/Documents/SWORD_Dev/outputs/'+version+'/netcdf/'+region.lower()+'_sword_'+version+'.nc'
# rch_dir = '/Users/ealteanau/Documents/SWORD_Dev/update_requests/v13/completed/EU_Reach_Removal.csv'
...
#     cl_ind1 = np.where(sword.groups['centerlines'].variables['reach_id'][0,:] == rm_rch.reach_id[ind])[0]
#     cl_ind2 = np.where(sword.groups['centerlines'].variables['reach_id'][1,:] == rm_rch.reach_id[ind])[0]
#     cl_ind3 = np.where(sword.groups['centerlines'].variables['reach_id'][2,:] == rm_rch.reach_id[ind])[0]
#     cl_ind4 = np.where(sword.groups['centerlines'].variables['reach_id'][3,:] == rm_rch.reach_id[ind])[0]

#     if len(cl_ind1) > 0:
#         sword.groups['centerlines'].variables['reach_id'][0,cl_ind1] = 0
#     if len(cl_ind2) > 0:
#         sword.groups['centerlines'].variables['reach_id'][1,cl_ind2] = 0
#     if len(cl_ind3) > 0:
#         sword.groups['centerlines'].variables['reach_id'][2,cl_ind3] = 0
#     if len(cl_ind4) > 0:
#         sword.groups['centerlines'].variables['reach_id'][3,cl_ind4] = 0
        
#     if len(cl_ind1) > 0 or len(cl_ind2) > 0 or len(cl_ind3) > 0 or len(cl_ind4) > 0:
#         print(rm_rch.reach_id[ind])

...
for f in list(range(len(mhv_nc_files))):
    ...
    for n in list(range(len(unq_nets))):
        net = np.where(mhv_network == unq_nets[n])[0]
        if max(mhv_add_flag[net]) < 3:
            join_fix.append(unq_nets[n])
            net_segs = np.unique(mhv_segs[net])
            if len(net_segs) > 1:
                mn_id = np.where(mhv_elv[net] == min(mhv_elv[net]))
                min_seg = np.unique(mhv_segs[mn_id])

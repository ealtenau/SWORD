...
import glob
import os
if os.path.exists('/Users/ealtenau/Documents/SWORD_Dev/src/SWORD/database_updates/channel_additions/global/'):
...
for f in list(range(len(mhv_nc_files))):
    ...
    # test = np.where(subcls.seg == xxx)[0]
    # test_sort = test[np.argsort(subcls.cl_id[test])]
    # print(np.array([subcls.reach_id[test_sort],subcls.new_reach_id[0,test_sort]]).T)

    ### define mhv topology.
    print('MHV Topology')
    mst.fill_mhv_topology(subcls)
    # test = np.where(subcls.reach_id == 71140200083)[0]
    # subcls.new_reach_id[1,test]
    # subcls.new_reach_id[2,test]
    # subcls.seg[test]
    # subcls.ind[test]

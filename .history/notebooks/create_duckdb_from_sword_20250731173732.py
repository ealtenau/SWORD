    ...
    with nc.Dataset(nc_path, 'r') as ds:
        ...
        for var_name, var in nodes_group.variables.items():
            ...
            if var_name == 'cl_ids' and len(var.dimensions) == 2:
                ...
                # Use .filled() to handle masked arrays from NetCDF
                nodes_data[var_name] = var[:].filled(np.nan) if hasattr(var[:], 'filled') else var[:]
        ...
        def extract_reaches_data(group, prefix=''):
            ...
            for var_name, var in group.variables.items():
                ...
                # Use .filled() to handle masked arrays, replacing fill values with NaN/None
                var_data = var[:].filled(np.nan) if hasattr(var[:], 'filled') else var[:]

...
water = np.where((type > 2) & (type < 5))[0]
ht_water = ht[water]
...
geom2 = gp.GeoSeries(map(Point, zip(lon_water, lat_water)))
pixc_water['geometry'] = geom2
...
outgpkg1 = fn_dir + 'gis_files/pixel_cloud_'+tile+'.gpkg'
outgpkg2 = fn_dir + 'gis_files/pixel_cloud_'+tile+'_water.gpkg'

pixc_all.to_file(outgpkg1, driver='GPKG', layer='pixc')
pixc_water.to_file(outgpkg2, driver='GPKG', layer='pixc')

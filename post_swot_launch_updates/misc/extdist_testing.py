...
modifier = 'yukon1_orthos10_extdist_5single_10multi_wth.gpkg'
...
#     if max_sig > 0:
#         ext_dist[npts] = (wth[npts]+(5*max_sig))/wth[npts]
#         if chan_med > 1:
#             update = np.where(ext_dist[npts] < 5)[0]
#             ext_dist[npts[update]] = 5
# sword['extdist'] = ext_dist

ext_dist[np.where(ext_dist < 5)] = 5

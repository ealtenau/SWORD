...
# Merging at level 2 basin scale
unq_l2 = np.unique(mhv_l2)
for ind in list(range(len(unq_l2))):
    start = time.time()
    
    # Defining mhv extent.
    l2 = np.where(mhv_l2 == unq_l2[ind])
    mhv_lon_clip = mhv_lon[l2]
    mhv_lat_clip = mhv_lat[l2]
    mhv_seg_clip = mhv_seg[l2]
    mhv_ext = [np.min(mhv_lon_clip), np.min(mhv_lat_clip),
                np.max(mhv_lon_clip), np.max(mhv_lat_clip)]

    # Creating geodataframe for spatial joins. 
    mhv_df = gp.GeoDataFrame([mhv_lon_clip, mhv_lat_clip]).T
    mhv_df.rename(columns={0:"x",1:"y"},inplace=True)
    mhv_df = mhv_df.apply(pd.to_numeric, errors='ignore')
    geom = gp.GeoSeries(map(Point, zip(mhv_lon_clip, mhv_lat_clip)))
    ...
    mhv_grand, mhv_grod, mhv_grodfid, mhv_hfallsfid = add_dams(mhv_lon_clip, mhv_lat_clip, fn_grand, fn_grod)
    mhv_basins = add_basins(mhv_df, fn_basins)
    mhv_deltas = add_deltas(mhv_df, mhv_seg_clip, delta_db)
    track_files = overlapping_tracks(mhv_lon_clip, mhv_lat_clip, track_list)
    mhv_numobs, mhv_orbs = add_tracks(mhv_df, track_files)

    # Filling in segment gaps with no basin values.
    mhv_new_basins = fill_zero_basins(mhv_lon_clip, mhv_lat_clip, mhv_seg_clip, mhv_basins)

    # Fill in local values. 
    mhv_lake_id[l2] = mhv_lakeid
    mhv_lakeflag[l2] = mhv_lakes
    mhv_deltaflag[l2] = mhv_deltas
    mhv_grand_id[l2] = mhv_grand
    mhv_grod_id[l2] = mhv_grod
    mhv_grod_fid[l2] = mhv_grodfid
    mhv_hfalls_fid[l2] = mhv_hfallsfid
    mhv_basin_code[l2] = mhv_new_basins
    mhv_number_obs[l2] = mhv_numobs
    mhv_orbits[l2,:] = mhv_orbs

    end = time.time()
    print('Finished Basin ' + str(unq_l2[ind]) + ' in: ' + str(np.round((end-start)/60, 2)) + ' min')
...
df.to_csv('/Users/ealteanau/Desktop/lake_merge_test.csv')
'''

...
def fetch_all_nodes_parallel(node_ids: List[str]) -> pd.DataFrame:
    ...
    with ProgressBar():
        df = ddf.compute(scheduler='threads', num_workers=32)
...
def quick_test_100_samples():
    ...
    try:
        with duckdb.connect(database=DUCKDB_FILE, read_only=True) as con:
            ...
            -- Upstream Swarm (Same Reach + All Parent Reaches)
            (
                -- Level 0: Same Reach
                SELECT obs.obstruction_node_id, n.node_id AS swarm_node_id, 'upstream' AS swarm_type,
                ...
                UNION
                -- Level 1: Parent Reaches
                SELECT obs_r.obstruction_node_id, n.node_id, 'upstream', n.x, n.y, n.dist_out, n.reach_id
            ...
            UNION ALL
            -- Downstream Swarm (Same Reach + All Child Reaches)
            (
                -- Level 0: Same Reach
                SELECT obs.obstruction_node_id, n.node_id AS swarm_node_id, 'downstream' AS swarm_type,
                       n.x, n.y, n.dist_out, n.reach_id
                FROM master_obstructions_df obs JOIN nodes n ON obs.reach_id = n.reach_id
                WHERE n.dist_out < obs.dist_out AND n.dist_out >= (obs.dist_out - 1500) AND n.lakeflag = 0
                UNION
                -- Level 1: Child Reaches
                SELECT obs_r.obstruction_node_id, n.node_id, 'downstream', n.x, n.y, n.dist_out, n.reach_id
                FROM obstruction_reaches obs_r JOIN nodes n 
            ...
            # Cap individual swarms to 10 nodes for manageable analysis
            logger.info("Capping individual swarms to 10 nodes...")
            MAX_NODES_PER_SWARM = 10
            swarm_map_df = (
                swarm_map_df
                .groupby(['obstruction_node_id', 'swarm_type'], group_keys=False)
                .apply(lambda g: g.sample(n=MAX_NODES_PER_SWARM, random_state=42) if len(g) > MAX_NODES_PER_SWARM else g)
                .reset_index(drop=True)
            )
            logger.info(f"Capped swarms to maximum {MAX_NODES_PER_SWARM} nodes per swarm")
...
def main():
    ...
    all_unique_nodes = swarm_map_df['swarm_node_id'].dropna().astype('int64').astype(str).unique().tolist()
    ...
    if time_series_df is None:
        ...
        if not time_series_df.empty: save_to_cache(cache_key, time_series_df)
    ...
    swarm_map_df['swarm_node_id'] = swarm_map_df['swarm_node_id'].astype('int64').astype(str)
    ...
    if 'swarm_balanced' in swarm_map_df.columns:
        ...
        analysis_df['swarms_were_balanced'] = analysis_df['swarms_were_balanced'].fillna(False)
    ...
    logger.info(f"Saved CSV analysis to {OUTPUT_CSV}")
    ...
    logger.info(f"Saved GeoJSON analysis to {OUTPUT_GEOJSON}")
    ...
    # Run quick test with 100 samples for verification

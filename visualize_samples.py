...
for m in re.finditer(pattern, log):
    ...
    pos = m.end()
    result_match = re.search(r'→ mean_drift=([0-9.]+)m, pct_ok=([0-9.]+)%, pct_major=([0-9.]+)%', log[pos:pos+500])
...
print(f"Success: {(df['status']=='success').sum()}, Failed: {(df['status']=='failed').sum()}")

# Create bins
success = df[df['status']=='success'].copy()
success['width_bin'] = pd.cut(success['width'], bins=[0,100,300,1000,10000], labels=['narrow','medium','wide','very_wide'])
success['slope_bin'] = pd.cut(success['slope'], bins=[0,1e-5,1e-4,1e-3,1], labels=['flat','low','medium','steep'])
success['nchan_bin'] = pd.cut(success['n_chan'], bins=[-1,1,2,5,100], labels=['single','double','multi','braided'])
...
success.boxplot(column='mean_drift', by='width_bin', ax=ax2, positions=[width_order.index(w) for w in success.groupby('width_bin').groups.keys()])
...
ax7.scatter(success['pct_ok'], success['mean_drift'], c=success['n_chan'], cmap='viridis', alpha=0.7, s=50)
...
good_counts = [len(success[(success['width_bin']==c) & (~success['is_outlier'])]) for c in categories]
outlier_counts = [len(success[(success['width_bin']==c) & (success['is_outlier'])]) for c in categories]
...
colors_sorted = ['green' if d < 60 else 'orange' if d < 500 else 'red' for d in sorted_success['mean_drift']]
ax9.barh(range(len(sorted_success)), sorted_success['mean_drift'], color=colors_sorted, alpha=0.7)
...
if len(good) > 0:
    ...
    ax10.axvline(good['mean_drift'].mean(), color='red', ls='-', lw=2, label=f"Mean: {good['mean_drift'].mean():.1f}m")
    ax10.axvline(good['mean_drift'].median(), color='blue', ls='--', lw=2, label=f"Median: {good['mean_drift'].median():.1f}m")
...
if len(failed) > 0:
    ...
    ax11.text(0.5, 0.5, 'No failures', ha='center', va='center', transform=ax11.transAxes)
...
plt.suptitle('SWORD Updater Stratified Sample Results (24 reaches)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
for _, r in df.iterrows():
    if r['status'] == 'success':
        status = 'GOOD' if r['mean_drift'] < 60 else 'MODERATE' if r['mean_drift'] < 500 else 'OUTLIER'
        print(f"{r['reach_id']:<15} {r['width']:>7.0f}m {r['slope']:>12.2e} {r['n_chan']:>7} {r['mean_drift']:>7.1f}m {r['pct_ok']:>5.1f}% {status:<10}")
    else:
        print(f"{r['reach_id']:<15} {r['width']:>7.0f}m {r['slope']:>12.2e} {r['n_chan']:>7} {'--':>8} {'--':>6} FAILED")

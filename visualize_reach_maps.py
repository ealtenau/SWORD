...
for i, reach_info in enumerate(selected):
    ...
    try:
        ...
        try:
            ...
            observed_utm = np.array([xform.transform(x, y) for x, y in result.observed_wgs84])
            observed_px = np.array([inv_t * (x, y) for x, y in observed_utm])[:, ::-1]

            # Transform proposed to pixel
            proposed_utm = np.array([xform.transform(x, y) for x, y in result.proposed_wgs84])
        ...
        ax.scatter(sword_px[0, 1], sword_px[0, 0], c='red', s=100, marker='^', zorder=11)  # Start
        ax.scatter(sword_px[-1, 1], sword_px[-1, 0], c='red', s=100, marker='v', zorder=11)  # End

        if has_observed:
            # Plot observed centerline
            ax.plot(observed_px[:, 1], observed_px[:, 0], 'g-', lw=2, label='Observed', zorder=9)
        ...
        # Title with stats
        status = 'GOOD' if reach_info['mean_drift'] < 60 else 'MOD' if reach_info['mean_drift'] < 500 else 'OUTLIER'

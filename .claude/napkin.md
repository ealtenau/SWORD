...
| 2026-02-21 | self | DuckDB APPROX_QUANTILE t-digest: ~3KB/digest. 18 quantiles × 2M groups = ~100GB hash state. threads=4 + 64GB OOM'd at 91GB RSS. threads=4 + 32GB also OOM'd. | Sub-chunk ID ranges (27 chunks) + threads=2 + memory_limit=32GB works. RSS stabilizes ~65%. |
| 2026-02-21 | code review | MAD from p80-p20 used wrong factor 0.7413 (IQR factor). p80-p20 = 1.6832σ, MAD = 0.6745σ → correct factor is 0.6745/1.6832 = **0.4010** | Always derive the scaling factor from the actual percentile spread used, not assume IQR |

...
def get_reach_linestring(reach_id, region):
    ...
    if (
        not result.empty
        and result["wkt"].iloc[0]
        and "EMPTY" not in result["wkt"].iloc[0]
    ):
...
def plot_sandwiched_reach_example(reach_id, region, output_path):
    ...
    for _, row in upstream.iterrows():
        ...
        if lons:
            ax.plot(
                lons,
                lats,
                color=colors.get(row["lakeflag"], "gray"),
                linewidth=max(2, min(8, row["width"] / 50)),
                alpha=0.7,
            )
            ax.scatter(
                [row["x"]],
                [row["y"]],
                c=colors.get(row["lakeflag"], "gray"),
                s=100,
                zorder=5,
                edgecolors="black",
            )
    ...
    try:
        cx.add_basemap(
            ax, crs="EPSG:4326", source=cx.providers.Esri.WorldImagery, zoom="auto"
        )
...
def plot_lakeflag_distribution(output_path):
    ...
    # Pie chart
    ax1.pie(
        df["count"], labels=df["type"], autopct="%1.1f%%", colors=colors, startangle=90
    )
...
def plot_sandwiched_examples_overview(output_path, n_examples=20):
    ...
    ax.set_title(
        f"River Reaches Sandwiched Between Lakes (Top {n_examples} by Length)\n"
        f"Total Found: {len(df)}",
        fontsize=14,
    )
    ax.set_xlabel("Longitude")
    ax.set_ylabel("Latitude")
...
def generate_all_materials():
    ...
    plot_sandwiched_reach_example(
        74269900341, "NA", OUTPUT_DIR / "example_lake_erie.png"
    )

    # 4. Detailed example: Russia
    print("\n4. Generating detailed example: Russia (26120600131)...")
    plot_sandwiched_reach_example(26120600131, "EU", OUTPUT_DIR / "example_russia.png")

    # 5. Detailed example: Amazon
    print("\n5. Generating detailed example: Amazon (62259000041)...")
    plot_sandwiched_reach_example(62259000041, "SA", OUTPUT_DIR / "example_amazon.png")

...
# Connect to SWORD
sword_db = duckdb.connect('/Users/jakegearon/projects/SWORD/data/duckdb/sword_v17b.duckdb', read_only=True)
...
for r in selected:
    status = 'GOOD' if r['mean_drift'] < 60 else 'MOD' if r['mean_drift'] < 500 else 'OUTLIER'
...
def generate_pipeline_figure(reach_info, sword_db, extractor, output_path):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with rasterio.open(
                ...
                crs=water_result.crs, transform=water_result.transform,
            ) as dst:
            ...
            if hasattr(riv, 'centerline') and riv.centerline is not None:
                ...
                if isinstance(cl, tuple) and len(cl) >= 2:
                    ...
                    rivgraph_cl_px = np.array([inv_t * (x, y) for x, y in cl_geo])[:, ::-1]
    ...
    if rivgraph_cl_px is not None:
        ax.plot(rivgraph_cl_px[:, 1], rivgraph_cl_px[:, 0], 'r-', lw=2, label='RivGraph')
    ax.set_title(f'7. Skeleton + RivGraph')
    ax.legend(fontsize=8)
    ax.axis('off')
    ...
    if rivgraph_cl_px is not None:
        ax.plot(rivgraph_cl_px[:, 1], rivgraph_cl_px[:, 0], 'b-', lw=1.5, alpha=0.7, label='Before')
    if snapped_cl_px is not None:
        ax.plot(snapped_cl_px[:, 1], snapped_cl_px[:, 0], 'g-', lw=1.5, label='After snap')
    ...
    if snapped_cl_px is not None:
        ...
        for i in range(0, len(sword_px), max(1, len(sword_px)//15)):
            ...
            ax.arrow(c, r, nearest[1] - c, nearest[0] - r,
    ...
    # Status
    status = 'GOOD' if reach_info['mean_drift'] < 60 else 'MODERATE' if reach_info['mean_drift'] < 500 else 'OUTLIER'

    plt.suptitle(f"Reach {reach_id} | drift={reach_info['mean_drift']:.0f}m | %ok={reach_info['pct_ok']:.0f}% | "

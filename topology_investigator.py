...
# Page config
st.set_page_config(
    page_title="SWORD Topology Investigator",
    page_icon="🔍",
    layout="wide"
)
...
def load_swot_summary(swot_path: str, region: str) -> pd.DataFrame:
    ...
    summary = swot_df.groupby('reach_id').agg({
        'wse': ['mean', 'std', 'count', 'min', 'max']
    }).reset_index()
    summary.columns = ['reach_id', 'wse_mean', 'wse_std', 'wse_count', 'wse_min', 'wse_max']
...
def get_flow_path(G: nx.DiGraph, start_node: int, direction: str = 'downstream', max_length: int = 20) -> List[int]:
...
def get_extended_path(G: nx.DiGraph, node1: int, node2: int, context: int = 5) -> List[int]:
...
def analyze_disagreements(reaches_df: pd.DataFrame, topo_df: pd.DataFrame,
                          swot_summary: pd.DataFrame, wse_threshold: float = 2.0) -> pd.DataFrame:
...
swot_path = st.sidebar.text_input("SWOT Path", "/Volumes/SWORD_DATA/data/swot/parquet_lake_D")
...
with st.spinner("Analyzing disagreements..."):
    disagree_df = analyze_disagreements(reaches_df, topo_df, swot_summary, wse_threshold)
...
with filter_col2:
    facc_filter = st.selectbox("facc Support", ["All", "facc also disagrees", "facc agrees"])
...
# Display table
display_cols = ['upstream', 'downstream', 'wse_diff', 'upstream_facc', 'downstream_facc',
                'facc_consistent', 'river_name']
st.dataframe(
...
if len(filtered) > 0:
    edge_options = [f"{row['upstream']} → {row['downstream']} (WSE diff: {row['wse_diff']:.2f}m)"
    ...
    if selected_edge:
            ...
            if len(valid_wse) > 0:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        error_y=dict(type='data', array=valid_wse['wse_std'], visible=True)
                    ),
                    row=1, col=1
                )
                ...
                sword_valid = display_df[(display_df['sword_wse'].notna()) & (display_df['sword_wse'] > 0)]
        ...
        with col2:
            ...
            with flip_col1:
                current_dir = "FLIPPED" if st.session_state.flip_direction else "CURRENT"
                st.write(f"Direction: **{current_dir}**")
            with flip_col2:
                if st.button("🔄 Flip Direction"):
                    st.session_state.flip_direction = not st.session_state.flip_direction
                    st.rerun()
            ...
            if len(map_df) > 0:
                ...
                def create_arrowhead(start_lon, start_lat, end_lon, end_lat, head_length=0.003):
                ...
                for i in range(len(map_df) - 1):
                    ...
                    is_selected_edge = bool((rid1 == upstream_id and rid2 == downstream_id) or \
                                      (rid1 == downstream_id and rid2 == upstream_id))
                    ...
                    if is_selected_edge and st.session_state.flip_direction:
                        ...
                        arrow_name = 'Current Flow' if not is_selected_edge else 'Selected Edge'
                    ...
                    arrowhead = create_arrowhead(start_x, start_y, end_x, end_y, head_size)
            ...
            edge_row = filtered[(filtered['upstream'] == upstream_id) & (filtered['downstream'] == downstream_id)].iloc[0]

            # Show what SWOT suggests
            swot_suggestion = "REVERSE (flip)" if not edge_row['swot_agrees'] else "KEEP current"
            swot_color = "🟠" if not edge_row['swot_agrees'] else "🟢"
            st.write(f"{swot_color} **SWOT suggests:** {swot_suggestion}")

            current_matches_swot = (st.session_state.flip_direction and not edge_row['swot_agrees']) or \
                                   (not st.session_state.flip_direction and edge_row['swot_agrees'])
            ...
            with btn_col1:
                if st.button("✅ Keep Current", type="primary" if not st.session_state.flip_direction else "secondary"):
            ...
            with btn_col2:
                if st.button("🔄 Flip Direction", type="primary" if st.session_state.flip_direction else "secondary"):
                    st.session_state.decisions[edge_key] = {
            ...
            if current_decision:
                ...
                if action == 'keep':
                    st.info(f"📌 Decision: KEEP current direction")
                elif action == 'flip':
                    st.info(f"📌 Decision: FLIP direction")
                elif action == 'skip':
                    st.info(f"📌 Decision: SKIPPED")
        ...
        with detail_col1:
            ...
            st.write(f"  - SWOT WSE: {edge_row['upstream_wse']:.2f} m (±{edge_row['upstream_std']:.2f})")
        ...
        with detail_col2:
            ...
            st.write(f"  - SWOT WSE: {edge_row['downstream_wse']:.2f} m (±{edge_row['downstream_std']:.2f})")
        ...
        st.write(f"**SWOT says:** {'Current direction correct' if edge_row['swot_agrees'] else '⚠️ Direction should be REVERSED'}")
        st.write(f"**facc says:** {'Current direction correct' if edge_row['facc_consistent'] else '⚠️ facc also suggests reversal' if edge_row['facc_consistent'] == False else 'Cannot determine'}")

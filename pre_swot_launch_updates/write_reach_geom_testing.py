...
def define_geometry(unq_rch, reach_id, cl_x, cl_y, cl_id, common, max_dist, region):
    ...
    for ind in list(range(len(unq_rch))):
        ...
        if common[ind] == 1: #not connecting any reach that is "common"
...
n_rch_up = data.groups['reaches'].variables['n_rch_up'][:]
n_rch_dn = data.groups['reaches'].variables['n_rch_down'][:]
...
up = np.where(n_rch_up > 1)[0]
dn = np.where(n_rch_dn > 1)[0]
common = np.zeros(len(unq_rch))
common[up] = 1
common[dn] = 1
...
geom, rm_ind = define_geometry(unq_rch, reach_id, cl_x, cl_y, 
                               cl_id, common, 500, region) #max_dist use to be 200...
end = time.time()

PRO GRWL_Tile_Fixes_v05
  ...
  openw, 1, 'C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\messages\GRWL_TileFix_Messages_NA.txt' ;text file that prints progress. 
  ...
  fix_path = 'C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\final_spreadsheets\GRWL_Edits_NA_Final_v2.csv' ;grwl edits directory.
  ...
  start_ind = 260 ;to run all rows this should be set to: 0.
  ...
  for ind=start_ind, end_ind do begin ; adjust start_ind and end_ind to debug when error occurs.
    ...
    fn_shp = shp_files[where(shp_names eq mask_names[ind])]
    ...
    count = 1
    start_rw = 0 ; edit for debugging
    end_rw = n_elements(rows)-1 ;edit for debugging
    
    for rw=start_rw, end_rw do begin

      ;if plot_figs eq 'True' then print, 'Row Number = ', rw, systime(1)-tm
      ...
      if flags[rows[rw]] eq 2 then begin
        
        if plot_figs eq 'True' then print, ind, ' starting linear interpolation'
        ...
        if flags[rows[rw]] eq 3 then clip_img = dilate(clip_img, operator)
    ...
    outpath = string('C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\csv\',region,'\',mask_names[ind],'_edit.csv') ;creating final outpath. 
    locs = where(fix_pts gt 0)
    
    if min(locs) lt 0 then begin
      printf, 1, FORMAT='("All Tile Boundary Fixes: ",A7)', mask_names[ind]
      continue
    endif

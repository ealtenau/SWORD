...
def read_data(filename):
    ...
    reaches.metroman_abar = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['Abar'][:]
    reaches.metroman_ninf = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['ninf'][:]
    reaches.metroman_p = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['p'][:]
    reaches.metroman_abar_stdev = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['Abar_stdev'][:]
    reaches.metroman_ninf_stdev = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['ninf_stdev'][:]
    reaches.metroman_p_stdev = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['p_stdev'][:]
    reaches.metroman_ninf_p_cor = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['ninf_p_cor'][:]
    reaches.metroman_ninf_abar_cor = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['ninf_Abar_cor'][:]
    reaches.metroman_p_abar_cor = data.groups['reaches']['discharge_models']['unconstrained']['MetroMan'].variables['p_Abar_cor'][:]

    reaches.bam_abar = data.groups['reaches']['discharge_models']['unconstrained']['BAM'].variables['Abar'][:]
    reaches.bam_n = data.groups['reaches']['discharge_models']['unconstrained']['BAM'].variables['n'][:]    
    
    reaches.hivdi_abar = data.groups['reaches']['discharge_models']['unconstrained']['HiVDI'].variables['Abar'][:]
    reaches.hivdi_alpha = data.groups['reaches']['discharge_models']['unconstrained']['HiVDI'].variables['alpha'][:]    
    reaches.hivdi_beta = data.groups['reaches']['discharge_models']['unconstrained']['HiVDI'].variables['beta'][:]    

    reaches.momma_b = data.groups['reaches']['discharge_models']['unconstrained']['MOMMA'].variables['B'][:]    
    reaches.momma_h = data.groups['reaches']['discharge_models']['unconstrained']['MOMMA'].variables['H'][:]    
    reaches.momma_save = data.groups['reaches']['discharge_models']['unconstrained']['MOMMA'].variables['Save'][:]    

    reaches.sads_abar = data.groups['reaches']['discharge_models']['unconstrained']['SADS'].variables['Abar'][:]    
    reaches.sads_n = data.groups['reaches']['discharge_models']['unconstrained']['SADS'].variables['n'][:]    
...
def reproject_utm(latitude, longitude):
    ...
    utm_let = np.unique(zone_let)

    pt_len = np.zeros(len(unq_zones))
    for idx in list(range(len(unq_zones))):
        pt_len[idx] = len(np.where(zone_num == unq_zones[idx])[0])

    idy = np.where(pt_len == np.max(pt_len))

    # Set the projection

    if np.sum(latitude) > 0:
        myproj = Proj(
		"+proj=utm +zone=" + str(int(unq_zones[idy])) + str(utm_let[idy][0]) +
		" +ellips=WGS84 +datum=WGS84 +units=m")
    else:
        myproj = Proj(
		"+proj=utm +south +zone=" + str(int(unq_zones[idy])) + str(utm_let[idy][0]) +
		" +ellips=WGS84 +datum=WGS84 +units=m")
...
# define paths. 
fn_lakes = 'E:/Users/Elizabeth Humphries/Documents/SWORD/lakes_near_rivers/shp/2367_dist_thresh_test.tif'
fn_nodes = 'E:/Users/Elizabeth Humphries/Documents/SWORD/lakes_near_rivers/databases/na_sword_scene2367_wc1_v06.nc'
outpath = 'E:/Users/Elizabeth Humphries/Documents/SWORD/lakes_near_rivers/s2367_dist_thresh_testing_extdvar.csv'
out_nc = 'E:/Users/Elizabeth Humphries/Documents/SWORD/lakes_near_rivers/databases/na_sword_scene2367_wc1_extdvar_v06.nc'
...
multi = np.where(nodes.nchan_max[final_ids] > 1)[0]
#dist_thresh[final_ids[multi]] = 2
#dist_thresh[final_ids[multi]] = nodes.nchan_max[final_ids[multi]]


nodes.ext_dist_coef = dist_thresh

#write csv.
data = pd.DataFrame(np.array([nodes.x, nodes.y, nodes.id, ntype, dist_thresh, nodes.nchan_max, nodes.wth])).T
data.columns = ['lon', 'lat', 'id', 'type', 'dist_thresh', 'num_channels', 'width']
data.to_csv(outpath)
...
'''
chans = np.where(nodes.nchan_max[final_ids] > 1)[0]

plt.scatter(nx, ny, c=dist_thresh, s = 5, edgecolors=None)
plt.scatter(lx[lake_locs], ly[lake_locs], c='red', s = 5, edgecolors=None)
plt.scatter(nx[final_ids[chans]], ny[final_ids[chans]], c='blue', s = 5, edgecolors=None)

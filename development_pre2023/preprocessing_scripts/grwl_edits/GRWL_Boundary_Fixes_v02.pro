PRO GRWL_Boundary_Fixes_v02
  ...
  openw, 1, 'C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\messages\GRWL_BoundaryFix_Messages_SA.txt' ;text file that prints progress.

  region = 'SA'

  mosaic_path = 'C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\temp_mos.tif' ;these paths create a temporary image is reprojected to match another. It will be overwritten. 
  prj_outpath = 'C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\temp_img.tif'
  warp_path = 'C:\Users\ealtenau\Anaconda2\Library\bin\gdalwarp.exe' ;!!!!! this path points to a GDAL tool used to reproject images. 
  ...
  fix_path = 'C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\final_spreadsheets\GRWL_Edits_SA_Update.csv' ;grwl edits directory.
  fixes = read_csv(fix_path, HEADER=FixHeader)
  ;help, fixes, /STRUCTURES ; shows information on the file structure
  grwl_names = fixes.FIELD06 ; may need to be "06" etc for some spreadsheets
  grwl_names2 = fixes.FIElD07
  longitude1 = fixes.FIELD02
  latitude1 = fixes.FIELD03
  longitude2 = fixes.FIELD04
  latitude2 = fixes.FIELD05
  flags = fixes.FIELD01

  ;assign indexes to run through.
  start_ind = 0 ;to run all rows this should be set to: 0.
  end_ind = n_elements(mask_names)-1 ; to run all rows this should be set to: n_elements(mask_names)-1.
  plot_figs = 'False'
  ...
  for ind=start_ind, end_ind do begin ; adjust start_ind and end_ind to debug when error occurs.
    ...
    count = 1
    start_rw = 0 ; edit for debugging. To run all values set to "0".
    end_rw = n_elements(rows)-1 ;edit for debugging. To run all values set to "n_elements(rows)-1".
    
    for rw=start_rw, end_rw do begin
      ...
      print, 'Tile Name = ', mask_names[ind], '; Tile Index = ', ind, '; Row Number = ', rw
      ...
      num1 = strmid(locs1.GTCITATIONGEOKEY, 20, 2)
      num2 = strmid(locs2.GTCITATIONGEOKEY, 20, 2)
      L1 = strmid(locs1.GTCITATIONGEOKEY, 22, 2)
      L2 = strmid(locs2.GTCITATIONGEOKEY, 22, 2)
      
      if utm_zone1 ne utm_zone2 then begin ;mosaicing is easier in same utm zones
        if L1 eq 'S' then prj1 = string('"+proj=utm +zone=', num1 +' +south +ellips=WGS84 +datum=WGS84"') else prj1 = string('"+proj=utm +zone=', num1 +' +ellips=WGS84 +datum=WGS84"')
        if L2 eq 'S' then prj2 = string('"+proj=utm +zone=', num2 +' +south +ellips=WGS84 +datum=WGS84"') else prj2 = string('"+proj=utm +zone=', num2 +' +ellips=WGS84 +datum=WGS84"')
      ...
      ;Identifying lat lon of points.
      lon1 = longitude1[rows[rw]] 
      lat1 = latitude1[rows[rw]] 
      lon2 = longitude2[rows[rw]] 
      lat2 = latitude2[rows[rw]] 
      ...
      if flags[rows[rw]] eq 6 then begin
        ...
        if plot_figs eq 'True' then begin
          new_img = new_pts
          new_img[where(grwl_img gt 0)] = 2
          new_img[where(new_pts gt 0)] = 3
          new_img[where(pt_mask eq 1)] = 5
          iimage, new_img, window_title = 'fixed locations with data: linear interpolation'
        endif
        ...
        if plot_figs eq 'True' then print, ind, ' starting mask interpolation'
        ...
        if plot_figs eq 'True' then iimage, clip_img, window_title = 'clipped image extent'
        ...
        if plot_figs eq 'True' then iimage, grwl_clip, window_title = 'clipped original grwl extent'
        ...
        if plot_figs eq 'True' then iimage, Cline, window_title = 'raw centerline'
        ...
        if plot_figs eq 'True' then iimage, Cline, window_title = 'pruned centerline'
        ...
        if plot_figs eq 'True' then iimage, fixes, window_title = 'fixed locations'

        if plot_figs eq 'True' then begin
          new_img = clip_img
          new_img[where(grwl_clip gt 0)] = 2
          new_img[where(fixes gt 0)] = 3
          new_img[where(cl_pts eq 1)] = 5
          iimage, new_img, window_title = 'fixed locations with data: mask interpolation'
        endif

        ;;; Putting new fixes in overal image extent.
    ...
    outpath = string('C:\Users\ealtenau\Documents\Research\SWAG\GRWL\EDITS\csv\',region,'\',mask_names[ind],'_bnds.csv')

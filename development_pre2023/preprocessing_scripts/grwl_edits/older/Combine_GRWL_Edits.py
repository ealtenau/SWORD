    ...
    for ind in list(range(len(uniq_segs))):
        ...
        if length < 100 and lakes == 0:
            ...
            if len(grwl_segs) > 0 and len(edt_segs) > 0:
                ...
                min_wth = np.where(wth == np.min(wth))[0]
...
def write_grwl_edits(grwl_lon, grwl_lat, grwl_x, grwl_y, grwl_Ind, grwl_ID, 
                     grwl_wth, grwl_lake, grwl_chan, grwl_manual, outfile):
    """
...
edit_paths = [file for file in getListOfFiles('C:/Users/ealtenau/Documents/Research/SWAG/GRWL/EDITS/csv/NorthAmerica/') if '.csv' in file]
...
for ind in list(range(len(grwl_paths))):
    ...
    write_grwl_edits(grwl_lon, grwl_lat, grwl_x, grwl_y, grwl_Ind, grwl_ID,\
                     grwl_wth, grwl_lake, grwl_chan, grwl_manual, outpath) 
    #end = time.time()
    #print('Time to Write the Data, ' + pattern + ': ' +str(np.round((end-start)/60, 3)) + ' min')
    
    end = time.time()
    print(pattern + ': Edits Combined in ' + str((end-start)/60) + 'min')
...
plt.xlabel('x (m)', fontsize=14)
plt.ylabel('y (m)', fontsize=14)
plt.scatter(edits_x, edits_y, c=endpoints, edgecolors='none', s = 5)
